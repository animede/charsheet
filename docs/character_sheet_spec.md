...
ComfyUI のワークフロー `$COMFYUI_DIR/workflow/character_sheet_v1.0.json` と同等の
処理を、実証済み PoC(`generate_qwen_edit.py`)の方式で Diffusers に移植する。
...
- **LoRA 2 本**(`load_lora_weights` を adapter_name 付きで 2 回 + `set_adapters` で両方有効化、weight 各 1.0):
  1. `$COMFYUI_DIR/models/loras/Qwen-Image-Edit-2511-Lightning-4steps-V1.0-bf16.safetensors`
  2. `$COMFYUI_DIR/models/loras/Qwen-Edit-2509-Multiple-angles.safetensors`
  (`$COMFYUI_DIR` は既定 `~/ComfyUI`。環境変数で上書き可。§13 参照)
...
- サーバー: FastAPI + uvicorn、ポート **8600**、専用 venv(`requirements.txt`)
...
- briaai/RMBG-1.4(ローカル `$COMFYUI_DIR/models/rmbg/RMBG-1.4/model.pth` があるケースを想定)は
...
## 13. モデル自動ダウンロード【追加実装済み 2026-07-08】

transformer / LoRA の各ファイルは、まず ComfyUI のローカルパス(pipeline.py 冒頭の
`*_PATH` 定数、`$COMFYUI_DIR/models/...`。`COMFYUI_DIR` は既定 `~/ComfyUI`、環境変数で
上書き可)を確認し、存在しなければ対応する Hugging Face リポジトリから自動的に
ダウンロードして通常の HF キャッシュ(`~/.cache/huggingface/hub/`)に保存する。

### 実装 (pipeline.py `_resolve_model_path`)

```python
def _resolve_model_path(local_path, repo_id, repo_filename):
...
| ローカル優先パス | HF リポジトリ / ファイル |
|---|---|
| `qwen_image_edit_2511_bf16.safetensors` | `Comfy-Org/Qwen-Image-Edit_ComfyUI` / `split_files/diffusion_models/qwen_image_edit_2511_bf16.safetensors` |
| `Qwen-Image-Edit-2511-Lightning-4steps-V1.0-bf16.safetensors` | `lightx2v/Qwen-Image-Edit-2511-Lightning` / `Qwen-Image-Edit-2511-Lightning-4steps-V1.0-bf16.safetensors` |
| `Qwen-Edit-2509-Multiple-angles.safetensors` | `Comfy-Org/Qwen-Image-Edit_ComfyUI` / `split_files/loras/Qwen-Edit-2509-Multiple-angles.safetensors` |
| `qwen_image_edit_2509_fp8_e4m3fn.safetensors`(フォールバック) | `Comfy-Org/Qwen-Image-Edit_ComfyUI` / `split_files/diffusion_models/qwen_image_edit_2509_fp8_e4m3fn.safetensors` |

2026-07-08 時点で、両 LoRA ファイルは HF からのダウンロード版とローカル ComfyUI 版が
md5 完全一致することを確認済み(transformer は容量が大きいためメタデータ存在確認のみ)。
