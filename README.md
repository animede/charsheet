...
- **VRAM 対策**: モデルロード時に空き VRAM を確認し、3段階で自動切り替えします
  (`CHARSHEET_OFFLOAD_MODE` 環境変数で明示指定も可: `none`/`group`/`group_lowvram`/`model_cpu`)。
  - 65GB 以上: 全常駐(最速)
  - 28GB 以上 65GB 未満(RTX 5090 32GB 等): transformer のみブロック単位で
    GPU/CPU を入れ替える `group` オフロード
  - 28GB 未満(RTX 4090 24GB 等): `group` に加えて text_encoder(bf16で~16GB)も
    使用直前/直後に GPU/CPU を入れ替える `group_lowvram`
  いずれも生成は遅くなります。ComfyUI 等の他プロセスが VRAM を大量使用している場合は
  事前に停止を推奨
